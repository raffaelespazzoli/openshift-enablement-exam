...
ssh -t `gcloud compute addresses list | grep ose-bastion | awk '{print $3}'` 'sudo subscription-manager attach --pool=$RHN_SUB_POOL && sudo subscription-manager refresh && sudo subscription-manager repos --disable="*" && sudo subscription-manager repos --enable="rhel-7-server-rpms" --enable="rhel-7-server-optional-rpms" --enable="rhel-7-server-extras-rpms" --enable="rhel-7-server-ose-3.3-rpms"'
...
ssh `gcloud compute addresses list | grep ose-bastion | awk '{print $3}'` 'cat /home/$username/.ssh/id_rsa.pub' >  ./id_rsa.pub
sed "s/^/$username:/" ./id_rsa.pub
